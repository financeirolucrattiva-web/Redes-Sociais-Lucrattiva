...
function extractContent(events) {
  ...
  // o agente varia o jeito de escrever o resumo (tabela, título solto, negrito
  // com dois-pontos...) — exige ":" ou "|" logo após o rótulo pra não confundir
  // a palavra usada numa frase qualquer com o rótulo de verdade
  function extractLabel(text, labelRe) {
    const pattern = new RegExp(
      `(?:^|\\n)[#*|\\s]*(?:${labelRe})(?:\\s+escolhid[ao])?\\s*\\**\\s*[:|]\\s*\\**\\s*([^\\n]+)`,
      "im"
    );
  ...
  function detectFormat(text, imageCount) {
    const near = text.match(/Formato[\s\S]{0,80}/i);
    const scope = near ? near[0] : text;
    if (/carross/i.test(scope)) return "Carrossel";
    if (/reels/i.test(scope)) return "Reels";
    if (/v[íi]deo/i.test(scope)) return "Vídeo";
    if (/imagem/i.test(scope)) return "Imagem";
    return imageCount > 1 ? "Carrossel" : "Imagem";
  }

  const theme = extractLabel(summary, "Tema") || "Conteúdo do dia";
  let network = field("Rede", "Instagram");
  network = network.replace(/\(.*/, "").trim() || "Instagram";
  const format = detectFormat(summary, images.length);
