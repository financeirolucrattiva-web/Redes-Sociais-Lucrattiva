...
async function listDeploymentRuns() {
  const data = await apiGet(`/v1/deployment_runs?beta=true&deployment_id=${DEPLOYMENT_ID}&limit=50`);
  return data.data || [];
}
...
function extractContent(events) {
  ...
  const theme = field("Tema", "Conteúdo do dia");
  let network = field("Rede", "Instagram");
  network = network.replace(/\(.*/, "").trim() || "Instagram";
  let format = field("Formato", images.length > 1 ? "Carrossel" : "Imagem");
  ...
  return { theme, network, format, time, caption, hashtags, imagesB64: images };
}

module.exports = { listDeploymentRuns, getSession, listAllEvents, extractContent };
